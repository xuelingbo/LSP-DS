...
def create_LDASIN_files(start_date, end_date, raw_data_dir, output_dir, geo_em_file, levelist, ZLVL, 
                        ahe_file=None, ahe_profile=None, utc_offset=None, pgw_file=False):
    ...
    pgw_delta = xr.open_dataset(pgw_file) if pgw_file else None

    # Pre-interpolate PGW deltas to the model domain (computed once, reused each timestep)
    _pgw_delta_map = {
        'ta':   ('T2D',      'add'),
        'hus':  ('Q2D',      'add'),
        'ua':   ('U2D',      'add'),
        ...
        'rlds': ('LWDOWN',   'add'),
        'rsds': ('SWDOWN',   'add'),
        'pr':   ('RAINRATE', 'multiply'),
    }
    pgw_domain = {}
    if pgw_delta is not None:
        for dvar, (lname, method) in _pgw_delta_map.items():
            if dvar not in pgw_delta:
                continue
            da = pgw_delta[dvar]
            if 'plev' in da.dims:
                da = da.sel(plev=da.plev.max())   # lowest model level (highest Pa)
            if da.dims == ():
                val = float(da.values)
                if method == 'multiply' and val == 0.0:
                    val = 1.0   # scalar 0 placeholder → no adjustment for ratio vars
                pgw_domain[lname] = (val, method)
                continue
            lat_dim = 'lat' if 'lat' in da.dims else 'latitude'
            lon_dim = 'lon' if 'lon' in da.dims else 'longitude'
            d_lat = da[lat_dim].values
            d_lon = da[lon_dim].values
            d_vals = da.values
            if d_lat[0] > d_lat[-1]:
                d_lat = d_lat[::-1]
                d_vals = d_vals[::-1, :]
            spl = RectBivariateSpline(d_lat, d_lon, d_vals, kx=1, ky=1)
            pgw_domain[lname] = (spl.ev(geo_lat_flat, geo_lon_flat).reshape(geo_lat.shape), method)
        ...
        for time in pd.date_range(date, periods=24, freq='h'):
            ...
            for var in variables:
                ...
                if variables[var]['name'] in ['T2D']:
                    ...
                    if 'T2D' in pgw_domain:
                        pgw_val, _ = pgw_domain['T2D']
                        data_var_correct_to_HGT_M = data_var_correct_to_HGT_M + pgw_val
                    ...
                    data_var_interpolated = interp_spline.ev(geo_lat_flat, geo_lon_flat).reshape(geo_lat.shape)
                    lname = variables[var]['name']
                    if lname in pgw_domain:
                        pgw_val, pgw_method = pgw_domain[lname]
                        if pgw_method == 'add':
                            data_var_interpolated = data_var_interpolated + pgw_val
                        elif pgw_method == 'multiply':
                            data_var_interpolated = data_var_interpolated * pgw_val
...
if __name__ == '__main__':
    ...
    ZLVL = 30
    utc_offset = 8
    pgw_file = '../hands-on/MIROC6/Tokyo/LDASIN/delta_historical_2009-2010_ssp585_2099-2100_08.nc'
